...
items query(int curr, int qL, int qR, int L, int R){
    ...
    items res;
    res.val = fir.val & sec.val;
    res.val |= tree[curr].OR;
    return res;
}
void update(int curr, int qL, int qR, ll val, int L, int R){
    if(qR<=L || qL>=R)
        return;
    if(qR>=R && qL<=L){
        tree[curr].OR  = (tree[curr].OR | val);
        tree[curr].val = (tree[curr].val | val);
        return;
    }
    int mid = (L+R)/2;
    update(2*curr, qL, qR, val, L, mid);
    update(2*curr+1, qL, qR, val, mid, R);
    tree[curr].val = tree[2*curr].val & tree[2*curr+1].val;
    tree[curr].val |= tree[curr].OR;
}
int main(){
    int n,q;
    cin>>n>>q;
